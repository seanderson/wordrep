...
char out1[7] = {'s','y','n','1','_','0','\0'};
char out0[7] = {'s','y','n','0','_','0','\0'};
...
// Returns position of a word in the vocabulary; if the word is not found, returns -int
...
//a wall time function used to measure how often a checkpoint should occur
...
//  printf("%ld\n",offset);
  for(i=0; i<MAX_CODE_LENGTH; i++){
    fwrite(&curr_word.code[i],sizeof(char),1,data);
  } 
  offset = ftell(data);
//  printf("%s","Write code stream ");
//  printf("%ld\n",offset);
  for(i = 0; i<MAX_CODE_LENGTH; i++){
    fwrite(&curr_word.point[i],sizeof(int),1,data);
  }
  offset = ftell(data);
//  printf("%s","Write point stream ");
//  printf("%ld\n",offset);   
  fwrite((&curr_word.cn),sizeof(long long),1,data);
  offset = ftell(data);
//  printf("%s","Write cn stream ");
//  printf("%ld\n",offset);
  fwrite((&curr_word.codelen),sizeof(curr_word.codelen),1,data);
  offset = ftell(data);
//  printf("%s","Write codelen stream ");
//  printf("%ld\n",offset);  
}
...
//Save *vocab to file 
int x = 0;
FILE *data = fopen("vocab.bin","wb");
for(x = 0; x<vocab_size; x++) WriteWordToFile(vocab[x],data);
fclose(data);
fclose(fo);
}
